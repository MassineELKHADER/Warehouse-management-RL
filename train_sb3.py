"""
Standalone SB3 training script — completely independent from agents/.

Uses Stable Baselines 3's battle-tested PPO and SAC implementations with our
WarehouseEnv wrapped as a Gymnasium env.
...
# ---------------------------------------------------------------------------
# Config loading (standalone copy — does not depend on train.py)
# ---------------------------------------------------------------------------
...
class WandbMetricsCallback:
    """
    Wraps SB3 callback interface to log episode metrics to WandB.
    Logs: episode_reward, transport_cost, demand_satisfaction, inventory_std.
    Also logs a shipping graph image every eval_every episodes.
    """
    ...
    def on_episode_end(self, model, episode: int = 0):
        ...
        if last_T is not None:
            try:
                ...
                self._run.log({"eval/shipping_graph": wandb.Image(fig)},
                              step=episode)
...
class _EpisodeCallback(BaseCallback):
    """
    Fire WandbMetricsCallback every `eval_every` episodes.

    Uses n_calls (total env steps across all parallel envs) rather than
    info-dict inspection, which is unreliable with SubprocVecEnv.
    """
    ...
    def __init__(self, metrics_cb, episode_length: int, eval_every: int, n_envs: int):
        ...
        self._eval_every     = eval_every
        self._n_envs         = n_envs
        # Steps between evals: eval_every episodes × steps_per_episode
        self._eval_interval  = eval_every * episode_length

    def _on_step(self) -> bool:
        # n_calls = total steps taken across ALL envs since training start
        if self.n_calls % self._eval_interval == 0:
            # Episode count approximation (across all parallel envs)
...
def main():
    ...
    parser.add_argument("--timesteps",  type=int, default=None,
                        help="Total environment timesteps (default: n_episodes * episode_length)")
    parser.add_argument("--no-wandb",   action="store_true")
    parser.add_argument("--n-envs",     type=int, default=8,
                        help="Number of parallel envs (default 8, use 1 to disable)")
    ...
    # SubprocVecEnv: each env runs in its own process → true CPU parallelism
    # Practical limit = number of physical CPU cores (no gain beyond that)
    env = make_vec_env(
        lambda: WarehouseGymEnv(cfg, seed=args.seed),
        n_envs=args.n_envs,
        vec_env_cls=SubprocVecEnv,
    )
    # VecNormalize: normalize rewards only (not obs — already normalised in _obs())
    # clip_reward=10 prevents huge negative spikes from destabilising value learning
    env = VecNormalize(env, norm_obs=False, norm_reward=True, clip_reward=10.0)

    # Single env for evaluation (keeps deterministic rollouts clean)
    ...
    if args.algo == "ppo":
        ...
        model = PPO(
            "MlpPolicy", env,
            learning_rate=3e-4,
            n_steps=episode_length,     # steps per env per update
            batch_size=256,
            n_epochs=10,
            gamma=0.99,
            gae_lambda=0.95,
